<script type="text/html" data-template-name="Omron Mobile Robot">
    <div class="form-row">
        <label style="width:150px" for="node-input-host"><i class="fa fa-tag"></i> IP</label>
        <input style="width:150px;text-align:center" type="text" id="node-input-host" placeholder="localhost">
    </div>
    <div class="form-row">
        <label style="width:150px" for="node-input-port"><i class="fa fa-tag"></i> PORT</label>
        <input style="width:50px;text-align:center" type="text" id="node-input-port" placeholder="502">
    </div>
    <div class="form-row">
        <label style="width:150px" for="node-input-password"><i class="fa fa-tag"></i> PASSWORD</label>
        <input style="width:150px;text-align:center" type="text" id="node-input-password" placeholder="***">
    </div>

    <div class="form-row">
        <label for="node-input-indexPLC"><i class="fa fa-dot-circle-o"></i>Index Mobile</label>
        <select id="node-input-indexPLC" style="width:80px; margin-right:5px;">
            <option value=1 data-i18n="indexPLC.1">1</option>
            <option value=2 data-i18n="indexPLC.2">2</option>
            <option value=3 data-i18n="indexPLC.3">3</option>
            <option value=4 data-i18n="indexPLC.4">4</option>
            <option value=5 data-i18n="indexPLC.5">5</option>
        </select>
    </div>
   <br/>
   <br/>
    <div class="form-row">
        <label for="node-input-name"><i class="fa fa-tag"></i> <span data-i18n="common.label.name"></span></label>
        <input type="text" id="node-input-name" data-i18n="[placeholder]Name">
    </div>
</script>

<script type="text/javascript">
    RED.nodes.registerType('Omron Mobile Robot', {
        category: 'Device',
        color: "White",
        defaults: {
            name: { value: "" },
            host: {
                value: "", required: true
            },
            password: {
                value: "", required: true
            },
            port: {
                value: "", required: true,
                label: RED._("node-red:tcpin.label.port"),
                validate: RED.validators.number(false)
            },
            indexPLC: {
                value: "", required: true,
                validate: RED.validators.number(false)
            },
        },
        inputs: 1,
        outputs: 1,
        icon: "mobile.png",
        label: function () {
            return this.name || "Mobile Robot:" + (this.host ? this.host : "") +' / '+ this.port;
        },
        labelStyle: function () {
            return this.name ? "node_label_italic" : "";
        }
    });
</script>