<!--
 Copyright (c) 2017,2018,2019,2020,2021,2022,2023,2024 Klaus Landsdorf (http://node-red.plus/)
 All rights reserved.
 node-red-contrib-modbus - The BSD 3-Clause License

 @author <a href="mailto:klaus.landsdorf@bianco-royal.de">Klaus Landsdorf</a> (Bianco Royal)
--> <script type='text/javascript'> RED.nodes.registerType('modbus-flex-server', {
    category: 'modbus',
    color: '#E9967A',
    defaults: {
      name: {value: ''},
  ...
  }) </script> <script type='text/x-red' data-template-name='modbus-flex-server'>
 <div class='form-row'> <ul style='background:#fff;min-width:600px;margin-bottom:20px' id='node-input-connector-tabs'>
</ul> </div> <div id='node-input-connector-tabs-content' style='min-height:170px'> <div id='modbus-server-tab-settings' 
style='display:none'> <div class='form-row'> <label for='node-input-name'><i class='icon-tag'></i> Name</label> <input 
type='text' id='node-input-name' placeholder='Name'> </div> <div class='form-row'> <label 
for='node-input-serverAddress'><i class='icon-tag'></i> <span data-i18n='modbus-contrib.label.address'></span></label> 
<input type='text' id='node-input-serverAddress' placeholder='0.0.0.0 (IPv4) or :: (IPv6)'> </div> <div 
class='form-row'> <label for='node-input-serverPort'><i class='icon-tag'></i> <span 
data-i18n='modbus-contrib.label.port'></span></label> <input type='text' id='node-input-serverPort' 
placeholder='2000 to 65536'> </div> <div class='form-row'> <label for='node-input-responseDelay'><i class='icon-tag'>
</i> <span data-i18n='modbus-contrib.label.responseDelay'></span></label> <input type='text' 
id='node-input-responseDelay' placeholder='100' style='max-width:120px'> <select id='node-input-delayUnit' 
style='max-width:160px'> <option value='ms'>millisecond(s)</option> <option value='s'>second(s)</option> <option 
value='m'>minute(s)</option> <option value='h'>hour(s)</option> </select> </div> <div class='form-row'> <label 
for='node-input-unitId'><i class='icon-tag'></i> <span data-i18n='modbus-contrib.label.unitId'></span></label> <input 
type='text' id='node-input-unitId' placeholder='1'> </div> <div class='form-row'> <label 
for='node-input-coilsBufferSize'><i class='icon-tag'></i> <span data-i18n='modbus-contrib.label.coils'></span></label> 
<input type='text' id='node-input-coilsBufferSize' placeholder='20000'> </div> <div class='form-row'> <label 
for='node-input-registersBufferSize'><i class='icon-tag'></i> <span data-i18n='modbus-contrib.label.registers'></span>
</label> <input type='text' id='node-input-registersBufferSize' placeholder='20000'> </div> <hr> <div class='form-row'> 
<label for='node-input-minAddress'><i class='icon-tag'></i> <span data-i18n='modbus-contrib.label.minAddress'></span>
</label> <input type='text' id='node-input-minAddress' placeholder='0'> </div> <div class='form-row'> <label 
for='node-input-splitAddress'><i class='icon-tag'></i> <span data-i18n='modbus-contrib.label.splitAddress'></span>
</label> <input type='text' id='node-input-splitAddress' placeholder='10000'> </div> <hr> <div class='form-row'> <label 
...
id='node-input-logEnabled' style='max-width:30px'> </div> </div> <div id='modbus-server-tab-getCoil' 
style='display:none'> <div class='form-row' style='margin-bottom:0'> <label for='node-input-funcGetCoil'><i 
class='fa fa-wrench'></i> <span data-i18n='function.label.function'></span></label> <input type='hidden' 
id='node-input-funcGetCoil'> </div> <div class='form-row node-text-editor-row'> <div 
style='height:250px;min-height:150px' class='node-text-editor' id='node-input-func-editor-getCoil'></div> </div> </div> 
<div id='modbus-server-tab-getDiscreteInput' style='display:none'> <div class='form-row' style='margin-bottom:0'> <label
 for='node-input-funcGetDiscreteInput'><i class='fa fa-wrench'></i> <span data-i18n='function.label.function'></span>
</label> <input type='hidden' id='node-input-funcGetDiscreteInput'> </div> <div class='form-row node-text-editor-row'> 
<div style='height:250px;min-height:150px' class='node-text-editor' id='node-input-func-editor-getDiscreteInput'></div> 
</div> </div> <div id='modbus-server-tab-getInput' style='display:none'> <div class='form-row' style='margin-bottom:0'> 
<label for='node-input-funcGetInputRegister'><i class='fa fa-wrench'></i> <span data-i18n='function.label.function'>
</span></label> <input type='hidden' id='node-input-funcGetInputRegister'> </div> <div 
class='form-row node-text-editor-row'> <div style='height:250px;min-height:150px' class='node-text-editor' 
id='node-input-func-editor-getInputRegister'></div> </div> </div> <div id='modbus-server-tab-getHolding' 
style='display:none'> <div class='form-row' style='margin-bottom:0'> <label for='node-input-funcGetHoldingRegister'><i 
class='fa fa-wrench'></i> <span data-i18n='function.label.function'></span></label> <input type='hidden' 
id='node-input-funcGetHoldingRegister'> </div> <div class='form-row node-text-editor-row'> <div 
style='height:250px;min-height:150px' class='node-text-editor' id='node-input-func-editor-getHoldingRegister'></div> 
</div> </div> <div id='modbus-server-tab-setCoil' style='display:none'> <div class='form-row' style='margin-bottom:0'> 
<label for='node-input-funcSetCoil'><i class='fa fa-wrench'></i> <span data-i18n='function.label.function'></span>
</label> <input type='hidden' id='node-input-funcSetCoil'> </div> <div class='form-row node-text-editor-row'> <div 
style='height:250px;min-height:150px' class='node-text-editor' id='node-input-func-editor-setCoil'></div> </div> </div> 
<div id='modbus-server-tab-setRegister' style='display:none'> <div class='form-row' style='margin-bottom:0'> <label 
for='node-input-funcSetRegister'><i class='fa fa-wrench'></i> <span data-i18n='function.label.function'></span></label> 
<input type='hidden' id='node-input-funcSetRegister'> </div> <div class='form-row node-text-editor-row'> <div 
style='height:250px;min-height:150px' class='node-text-editor' id='node-input-func-editor-setRegister'></div> </div> 
</div> </div> </script> <script type='text/x-red' data-help-name='modbus-flex-server'>
 <p>Node to provide a flexible Modbus TCP server based on modbus-serial for testing.</p> <p>
 The modbus-serial package allows to configure/inject code to handle modbus requests. With that you can build you personal Modbus server. The function frame is fix - just the body of functions is flexible. 
</p> <p>On injecting the server sends the Buffers to the separate outputs</p> <p>
...
Output 4: discrete inputs Buffer, type, msg</p> <br> <p>
...
<code>
 ...
 Set disableMsgOutput if you want to disable the Flex Server outputs when injecting. <p></p> </script> 