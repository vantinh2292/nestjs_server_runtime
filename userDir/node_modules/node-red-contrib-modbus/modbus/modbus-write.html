...
id='node-input-name' placeholder='Name'> </div> <div class='form-row'> <label for='node-input-unitid'><i 
...
type='text/x-red' data-help-name='modbus-write'> <p><strong>
 If you have more than 10 nodes on one communication configuration, use the Modbus-Flex-Writer or think about multiple connections to your Modbus device, please! 
</strong></p> <p>Modbus TCP/Serial node triggered with <strong>msg.payload</strong> to write.</p> <p>
Connects to a Modbus TCP or serial to write coils/registers at each incoming msg.</p> <p> </p><h4>
Function codes currently supported include:</h4> <ul> <li>FC 5: Force Single Coil</li> <li>FC 6: Preset Single Register
</li> <li>FC 15: Force Multiple Coils</li> <li>FC 16: Preset Multiple Registers</li> </ul> <p></p> <p>
 Choose a function code (FC) from the dropdown menu, select the coil/register start address (0:65535) and the quantity to write. Choose or edit the Modbus TCP/serial server configuration to specify the server to connect to. 
</p> <p>Unit-Id (0..255 tcp | 0..247 serial) - leave empty, otherwise it overrides the default Unit-ID of client config
</p> <p> For FC 5, <strong>msg.payload</strong> must be a value of 1 or 0 or true or false. For FC 15, <strong>
msg.payload</strong> must be an array[] of comma separated values true or false each. For FC 6, <strong>msg.payload
...
Output 2: modbus response Buffer, all given data, input message</p> </script> 