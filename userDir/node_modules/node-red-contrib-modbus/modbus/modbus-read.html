...
--> <script type='text/javascript'> RED.nodes.registerType('modbus-read', {
  ...
  }) </script> <script type='text/x-red' data-template-name='modbus-read'>
...
data-i18n='modbus-contrib.label.unitId'></span></label> <input type='text' id='node-input-unitid'> </div> <div 
class='form-row'> <label for='node-input-dataType'><i class='icon-list'></i> <span 
data-i18n='modbus-contrib.label.functioncode'></span></label> <select id='node-input-dataType'> <option value='Coil'>
FC 1: Read Coil Status</option> <option value='Input'>FC 2: Read Input Status</option> <option value='HoldingRegister'>
FC 3: Read Holding Registers</option> <option value='InputRegister'>FC 4: Read Input Registers</option> </select> </div>
 <div class='form-row'> <label for='node-input-adr'><i class='icon-home'></i> <span 
data-i18n='modbus-contrib.label.address'></span></label> <input type='text' id='node-input-adr' placeholder='0:65535'> 
</div> <div class='form-row'> <label for='node-input-quantity'><i class='icon-bookmark'></i> <span 
data-i18n='modbus-contrib.label.quantity'></span></label> <input type='text' id='node-input-quantity' 
placeholder='1-65535'> </div> <div class='form-row'> <label for='node-input-rate'><i class='icon-bookmark'></i> <span 
data-i18n='modbus-contrib.label.pollrate'></span></label> <input type='number' id='node-input-rate' 
placeholder='1-65535' style='max-width:120px'> <select id='node-input-rateUnit' style='max-width:160px'> <option 
value='ms'>millisecond(s)</option> <option value='s'>second(s)</option> <option value='m'>minute(s)</option> <option 
value='h'>hour(s)</option> </select> </div> <div class='form-row' id='node-delayonstart'> <label 
...
id='node-input-startDelayTime' placeholder='10' style='width:80px'> </div> <div class='form-row'> <label 
...
class='form-row'> <label style='min-width:190px' for='node-input-emptyMsgOnFail'><i class='fa fa-th'></i> <span 
...
</div> </script> <script type='text/x-red' data-help-name='modbus-read'> <p><strong>
 If you have more than 10 nodes on one communication configuration, use the Modbus-Flex-Getter or think about multiple connections to your Modbus device, please! You can do polling with the Modbus-Getter and Modbus-Flex-Getter, too by using an injecting node with an interval. 
</strong></p> <p>Modbus TCP/Serial read node.</p> <p>
Connects to a Modbus TCP or serial to read registers/coils values with a given polling rate.</p> <p> </p><h4>
...
<li>FC 3: Read Holding Registers</li> <li>FC 4: Read Input Registers</li> </ul> <p></p> <p>
 Choose a function code (FC) from the dropdown menu, select the coil/input/register start address (0:65535), and the quantity of coils/inputs/registers to be read from the start address. 
</p> <p>Unit-Id (0..255 tcp | 0..247 serial) - leave empty, otherwise it overrides the default Unit-ID of client config
</p> <p>
 Setup a polling rate (greater than zero) and the time unit. Choose or edit the Modbus connection configuration. </p> 
<p>Output 1: data Array (PDU), modbus response Buffer, input message</p> <p>
