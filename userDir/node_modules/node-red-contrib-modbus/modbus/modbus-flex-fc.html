...
<div class='form-row'> <label for='node-input-name'><i class='icon-tag'></i> Name</label> <input type='text' 
id='node-input-name' placeholder='Name'> </div> <div class='form-row'> <label for='node-input-unitid'>Unit ID</label> 
...
data-i18n='modbus-contrib.label.mapPath'> </span></label> <input type='text' id='node-input-mapPath' 
placeholder='./node_modules/node-red-contrib-modbus/extras/argumentMaps/defaults/'> </div> <div class='form-row'> <label
 for='node-input-selected-fc'><i class='icon-list'></i> <span data-i18n='modbus-contrib.label.functioncode'></span>
</label> <select id='node-input-selected-fc'> <option value=''>Select Map</option> </select> <button 
id='node-input-reset-request' type='button' class='red-ui-button'>Reset Maps</button> </div> <div class='form-row'> 
<label for='node-input-request-map'><span data-i18n='modbus-contrib.label.requestMap'></span></label> <div 
style='height:250px;min-height:150px' class='node-text-editor' id='node-input-request-map'></div> </div> <div 
class='form-row'> <label for='node-input-response-map'><span data-i18n='modbus-contrib.label.responseMap'></span>
</label> <div style='height:250px;min-height:150px' class='node-text-editor' id='node-input-response-map'></div> </div> 
</div> <div id='modbus-options-tab' style='display:none'> <div class='form-row'> <label style='min-width:190px' 
...
type='text/x-red' data-help-name='modbus-flex-fc'>
 <p>Transmits and Recieves Answers for Nonstandard Functioncodes via MODBUS </p> <p>
Unit-ID - The Device Identification the Node tries to reach via MODBUS.</p> <p>
 Choose a Custom Functioncode from the dropdown. The editors will then load the Argument Maps for this certain Code. 
</p> <p>
 After selecting a Custom Functioncode you can make changes to the loaded ArgumentMaps. If you made a mistake and want to reload the template values click the 
<strong>'Reset Maps'</strong> button this will reload all values from the template data. </p> <p>
 Every Custom Functioncode has two possible Argument Maps one for the Request and one for the Response part of the code. These get loaded in their respective Editors where you can make changes to the 
<strong>names,values, offsets or types</strong> of the arguments. </p> <p><strong>Request Map Editor -</strong>
 Data editor for the request of the custom function code.</p> <p><strong>Response Map Editor -</strong>
 Data editor for the response of the custom function code.</p> <p> layout of a argument map: </p><ul> <li><strong>Name -
</strong> The Name of the Argument. On the <strong>Request side</strong>
 this will only be used for readability purposes, on the <strong>Response side</strong>
 the name is used to generate a valid JSON response object.</li> <li><strong>Data -</strong>
 The Data value. The Argument get set to this value in the <strong>request</strong>. Has no effect on the <strong>
response</strong> and should be set to <strong>zero</strong></li> <li><strong>Offset -</strong>
 The Offset of the Data Value. The Offset is used by the Parser to put the Data in the right place of the function code. The 0 offset is the first argument after the functioncode.
</li> <li><strong>Type -</strong> The Datatype. In the <strong>Request</strong>
 this type will be written into the datastream. In the <strong>Response</strong>
 however this type will be read from the stream.</li> <ul> <li><strong>uint8be -</strong>
 The Argument is a 8Bit Unsigned Integer.</li> <li><strong>uint16be -</strong>
 The Argument is a 16Bit Unsigned Integer encoded as Big Endian.</li> <li><strong>uint32be -</strong>
 The Argument is a 32Bit Unsigned Integer encoded as Big Endian.</li> </ul> </ul> <p></p> <p>
 Example of a custom function code message: <code>
